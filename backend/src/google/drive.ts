 ...
 * ingest source — see SPEC §8.7 for the rationale and the matrix of which
 ...
 * the V2 empirical check in `docs/roadmap.md` to verify that uploading a known
...
export interface DriveCommentAuthor {
   ...
   * name (OOXML doesn't surface this — SPEC §8.7). Hashed and stored as
...
export async function stopChannel(
  ...
  if (!res.ok && res.status !== 404) {
    throw new GoogleApiError(res.status, `${DRIVE_BASE}/channels/stop`, await res.text());
  }
}
